...
import { getPrivateAssetCurrentTotals, getPrivateAssetHistoricalSeries, listPrivateAssetRows } from "@/adapters";
import { useBalancePrivacy } from "@/hooks/use-balance-privacy";
import { QueryKeys } from "@/lib/query-keys";
import { useSettingsContext } from "@/lib/settings-provider";
import type { PrivateAssetCurrentTotals, PrivateAssetHistoricalPoint, PrivateAssetListRow } from "@/lib/types";
    ...
    <>
      <div className="space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <SummaryCard
            title="Current Value"
            value={formatPrivateAmount(totalsQuery.data?.totalCurrentValue, chartCurrency, isBalanceHidden)}
            subtitle="Latest reported value only"
            isLoading={totalsQuery.isLoading}
          />
          <SummaryCard
            title="Contributed"
            value={formatPrivateAmount(totalsQuery.data?.totalContributed, chartCurrency, isBalanceHidden)}
            subtitle="From latest statements"
            isLoading={totalsQuery.isLoading}
          />
          <SummaryCard
            title="Distributed"
            value={formatPrivateAmount(totalsQuery.data?.totalDistributed, chartCurrency, isBalanceHidden)}
            subtitle="From latest statements"
            isLoading={totalsQuery.isLoading}
          />
          <SummaryCard
                    ...
                    {latestHistoryRows.map((point) => (
                      <TableRow key={point.asOfDate}>
                        ...
                        <TableCell className="text-right">
                          {formatPrivateAmount(point.totalCurrentValue, chartCurrency, isBalanceHidden)}
                        </TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Freshness comes from the latest statement. Rows without a statement stay visible but do not add value yet.
            </CardDescription>
          </CardHeader>
          <CardContent>
            {isLoading ? (
                    ...
                    <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                      <div className="space-y-2">
                        ...
                        <div className="text-muted-foreground flex flex-wrap gap-x-4 gap-y-1 text-sm">
                          ...
                          {row.latestSnapshot ? (
                            <span>
                              {formatPrivateSnapshotValueSource(row.latestSnapshot.valueSourceType)} mark
                            </span>
                          ) : (
                      ...
                      <div className="grid min-w-[260px] gap-2 text-sm sm:grid-cols-2 lg:text-right">
                        <div>
                          ...
                          <div className="font-semibold">
                            {row.latestSnapshot
                              ? formatPrivateAmount(row.latestSnapshot.currentValue, row.currency, isBalanceHidden)
                              : "—"}
                          </div>
                        </div>
                        <div>
                          <div className="text-muted-foreground">Latest As-Of</div>
                          <div className="font-semibold">{formatDate(row.latestSnapshot?.asOfDate)}</div>
                        </div>
                        <div>
                          ...
                          <div className="font-semibold">
                            {row.latestSnapshot
                              ? formatPrivateAmount(row.latestSnapshot.contributedAmount, row.currency, isBalanceHidden)
                              : "—"}
                          </div>
                        </div>
                        <div>
              ...
              <EmptyPlaceholder>
                ...
                <EmptyPlaceholder.Description>
                  Start by adding a private asset, then add its first statement from the detail page.
                </EmptyPlaceholder.Description>
