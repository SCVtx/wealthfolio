...
export default function PrivateAssetDetailPage() {
  ...
  if (!detail) {
    return (
      <div className="space-y-6">
        <SettingsHeader heading="Private Asset" text="Detail view" backTo="/settings/private-assets" />
        <Separator />
        <EmptyPlaceholder>
    ...
    <>
      <div className="space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <DetailMetric
            label="Current Value"
            value={latestSnapshot ? formatPrivateAmount(latestSnapshot.currentValue, detail.asset.currency, isBalanceHidden) : "—"}
          />
          <DetailMetric
            label={
              latestSnapshot
            ...
            value={latestSnapshot ? formatPrivateAmount(latestSnapshot.contributedAmount, detail.asset.currency, isBalanceHidden) : "—"}
          />
          <DetailMetric
            label={
              latestSnapshot
            ...
            value={latestSnapshot ? formatPrivateAmount(latestSnapshot.distributedAmount, detail.asset.currency, isBalanceHidden) : "—"}
          />
          <DetailMetric label="Latest As-Of" value={formatDate(latestSnapshot?.asOfDate)} />
        </div>

        <div className={`grid gap-4 ${showManagerCard ? "xl:grid-cols-[minmax(0,1.25fr)_minmax(0,1fr)]" : ""}`}>
          <Card>
            <CardHeader>
            ...
            <CardContent className="grid gap-4 sm:grid-cols-2">
              ...
              <MetadataItem label="Vehicle Kind" value={formatPrivateAssetVehicleKind(detail.asset.vehicleKind)} />
              <MetadataItem label="Strategy" value={formatPrivateAssetStrategy(detail.asset.strategyType)} />
              <MetadataItem label="Commitment" value={formatPrivateAmount(detail.asset.commitmentAmount, detail.asset.currency, isBalanceHidden)} />
              ...
              <div className="sm:col-span-2">
                ...
                <div className="font-medium">{detail.asset.notes?.trim() ? detail.asset.notes : "—"}</div>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="space-y-3">
              {latestSnapshot ? (
                <>
                  ...
                  <MetadataItem
                    label="Current Value"
                    value={formatPrivateAmount(latestSnapshot.currentValue, detail.asset.currency, isBalanceHidden)}
                  />
                  <MetadataItem
                    label={getPrivateStatementAmountLabel(latestSnapshot.cashFlowType, "contributed")}
                    value={formatPrivateAmount(latestSnapshot.contributedAmount, detail.asset.currency, isBalanceHidden)}
                  />
                  <MetadataItem
                    label={getPrivateStatementAmountLabel(latestSnapshot.cashFlowType, "distributed")}
                ...
                <p className="text-muted-foreground text-sm">No statements have been recorded yet.</p>
              )}
            </CardContent>
          </Card>
        </div>
                    ...
                    {[...snapshots]
                      ...
                      .map((snapshot) => (
                        <TableRow key={snapshot.id}>
                          ...
                          <TableCell>{formatPrivateSnapshotCashFlowType(snapshot.cashFlowType)}</TableCell>
                          <TableCell className="text-right">
                            {formatPrivateAmount(snapshot.contributedAmount, detail.asset.currency, isBalanceHidden)}
                          </TableCell>
                          <TableCell className="text-right">
                            {formatPrivateAmount(snapshot.distributedAmount, detail.asset.currency, isBalanceHidden)}
                          </TableCell>
                          <TableCell className="text-right">
                            {formatPrivateAmount(snapshot.currentValue, detail.asset.currency, isBalanceHidden)}
                          </TableCell>
                          <TableCell className="text-right">
                            <Button
                              variant="ghost"
                    ...
                    <TableBody>
                      {detail.subAssets.map((subAsset) => (
                        <TableRow key={subAsset.id}>
                          ...
                          <TableCell>{formatPrivateSubAssetReportingBasis(subAsset.reportingBasis)}</TableCell>
                          <TableCell>{subAsset.strategyType ? formatPrivateAssetStrategy(subAsset.strategyType) : "—"}</TableCell>
                          <TableCell className="text-right">
                            {formatPrivateAmount(subAsset.currentValue, detail.asset.currency, isBalanceHidden)}
                          </TableCell>
                          <TableCell className="text-right">
