...
export const HoldingsInsightsPage = ({ accountId: accountIdProp }: HoldingsInsightsPageProps) => {
  ...
  const showPrivateAssetsSection = accountId === PORTFOLIO_ACCOUNT_ID;
  ...
  const isLoading =
    holdingsLoading || allocationsLoading || (showPrivateAssetsSection && privateAssetsQuery.isLoading);
  ...
  const hasNoHoldingsAtAll = !isLoading && (!holdings || holdings.length === 0) && !hasPrivateAssets;
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {PRIVATE_FRESHNESS_ORDER.map((state) => {
                    ...
                    return (
                      <Badge key={state} variant="outline" className={getFreshnessBadgeClass(state)}>
