...
import { DateRange, TimePeriod, type NetWorthHistoryPoint, type PrivateAssetHistoricalPoint } from "@/lib/types";
...
export function DashboardContent() {
  ...
  const { latestValuations } = useLatestValuations([PORTFOLIO_ACCOUNT_ID]);
  ...
  const dashboardHistory = useMemo((): DashboardHistoryPoint[] => {
    ...
    const getPrivateNetContribution = buildPrivateFlowLookup(privateHistoryQuery.data ?? []);

    return netWorthHistory.map((point: NetWorthHistoryPoint) => ({
      date: point.date,
      totalValue: parseDecimal(point.portfolioValue) + parseDecimal(point.privateAssetsValue),
  ...
  }, [baseCurrency, netWorthHistory, privateHistoryQuery.data]);
  ...
  return (
    <div className="flex min-h-screen flex-col">
      <div className="px-4 pb-1 pt-2 md:px-6 md:pb-2 lg:px-8">
        <PortfolioUpdateTrigger lastCalculatedAt={latestPortfolioValuation?.calculatedAt}>
