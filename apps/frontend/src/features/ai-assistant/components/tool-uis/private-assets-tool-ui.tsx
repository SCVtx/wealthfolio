...
function normalizeDetailResult(result: unknown): GetPrivateAssetDetailResult | null {
  ...
  return {
    privateAssetId: asOptionalString(candidate.privateAssetId ?? candidate.private_asset_id) ?? undefined,
    detail: {
      asset: {
        ...
        commitmentAmount: asset.commitmentAmount ?? asset.commitment_amount
          ? asNumber(asset.commitmentAmount ?? asset.commitment_amount)
          : null,
      },
      fundManager: detail.fundManager && typeof detail.fundManager === "object"
        ? { name: asString((detail.fundManager as Record<string, unknown>).name, "Unknown manager") }
        : null,
...
export const PrivateAssetRowsToolUI = makeAssistantToolUI<Record<string, never>, ListPrivateAssetRowsResult>({
...
export const PrivateAssetDetailToolUI = makeAssistantToolUI<{ privateAssetId: string }, GetPrivateAssetDetailResult>({
...
function PrivateAssetDetailContent({
  ...
  const { asset, fundManager, latestSnapshot, freshnessState, snapshots, subAssets } = parsed.detail;
  ...
  return (
    ...
    >
      <div className="grid gap-3 sm:grid-cols-2">
        <MetricCard label="Current Value" value={formatCurrency(latestSnapshot?.currentValue ?? null, asset.currency)} />
        <MetricCard label="Latest As-Of" value={latestSnapshot?.asOfDate ? formatDate(latestSnapshot.asOfDate) : "—"} />
        <MetricCard label="Contributed" value={formatCurrency(latestSnapshot?.contributedAmount ?? null, asset.currency)} />
        <MetricCard label="Distributed" value={formatCurrency(latestSnapshot?.distributedAmount ?? null, asset.currency)} />
      </div>
...
export const PrivateAssetTotalsToolUI = makeAssistantToolUI<{ includeArchived?: boolean }, GetPrivateAssetCurrentTotalsResult>({
  ...
  return (
    ...
    >
      <div className="grid gap-3 sm:grid-cols-2">
        ...
        />
        <MetricCard label="Latest As-Of" value={totals.latestAsOfDate ? formatDate(totals.latestAsOfDate) : "—"} />
      </div>
    </ToolShell>
  );
}

export const PrivateAssetHistoryToolUI = makeAssistantToolUI<{ includeArchived?: boolean }, GetPrivateAssetHistoricalSeriesResult>({
